 ...
 * return_to（外部URL禁止）
 */
$returnTo = (string)($_GET['return_to'] ?? '/admin/index.php');
if ($returnTo === '' || $returnTo[0] !== '/') $returnTo = '/admin/index.php';
if (strpos($returnTo, '/admin/') !== 0) $returnTo = '/admin/index.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isValidCsrf((string)($_POST['csrf_token'] ?? ''))) {
        ...
        if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            $stmt = $pdo->prepare("
            SELECT id, tenant_id, password_hash, email
 ...
 */
$logoUrl = '../images/logo_main.png?v=' . date('YmdHis');
?>
<!doctype html>
<html lang="ja">
